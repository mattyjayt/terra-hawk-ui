...
| **Technical Design** | `LiveStream.tsx` initiates a WHEP (WebRTC-HTTP Egress Protocol) handshake: creates an `RTCPeerConnection` with STUN ICE servers, adds a `recvonly` video transceiver, generates an SDP offer, waits for ICE gathering, POSTs the SDP to the WHEP endpoint (`VITE_WHEP_URL`), and sets the SDP answer as remote description. `ontrack` event captures the `MediaStream` and attaches it to a `<video>` element. 10-second timeout — if no track is received, falls back to a looping `sim-terrarium.mp4`. Status exposed via `onStatusChange` callback: `idle → connecting → live \| simulated`. Stats (FPS, jitter buffer latency) reported via `onStats` callback using `RTCPeerConnection.getStats()`. **Production architecture:** WHEP signaling routed through Cloudflare Tunnel (`rtc.terra-hawk.com` → Pi port 8889). WebRTC media relayed via self-hosted `coturn` TURN server on Pi (port 3478, static-auth-secret) to traverse NAT/firewall. MediaMTX `webrtcICEServers2` configured with the local TURN server. **Achieved latency: ~10ms** (down from ~8000ms with previous HLS approach). HLS kept as fallback on `stream.terra-hawk.com` (port 8888) but no longer used by the frontend |
| **Acceptance Criteria** | ① Live stream renders within 10s when Pi is reachable ② Fallback video plays within 10s when Pi is offline ③ Status indicator shows correct state (live/connecting/simulated) ④ No console errors or broken UI in either path ⑤ Transition between live and simulated is visually seamless ⑥ Stream latency < 100ms through Cloudflare Tunnel ⑦ WebRTC stats (FPS, latency) displayed in HUD |
| **Edge Cases** | `VITE_WHEP_URL` not set → immediate fallback to simulated. WHEP POST returns non-200 → fallback. ICE negotiation fails → `connectionState` becomes `"failed"`, triggers fallback. Stream drops mid-session → `connectionState` becomes `"disconnected"`, triggers fallback. TURN server down → WebRTC may still work via direct STUN (LAN only), fails through tunnel → fallback. coturn restart required after Pi reboot (systemd enabled) |
| **Dependencies** | FR-001 (MediaMTX running on Pi), `VITE_WHEP_URL` env var configured, STUN server reachable (`stun.l.google.com:19302`), coturn running on Pi (port 3478), Cloudflare Tunnel route for `rtc.terra-hawk.com` → localhost:8889 |
...
| **Technical Design** | `mqtt_client.py` creates a `paho.mqtt.Client`, connects to **HiveMQ Cloud** (TLS, port 8883) with username/password authentication, subscribes to `terrahawk/+/sensors` (wildcard). `on_message` callback parses JSON payloads, routes by system ID extracted from the topic, and updates the per-system `sensor_state` dict with `status`, `temperature`, `humidity`, and `soil`. The MQTT client is instantiated at FastAPI module import and runs `loop_start()` in a background thread. Outbound topic `terrahawk/{system_id}/commands` is used for Pi → ESP32 commands. ESP32 publishes every 30s with DHT11 (pin 18) + soil moisture (analog pin 36) data |
| **Acceptance Criteria** | ① Sensor data received and parsed within 1s of ESP32 publish ② `sensor_state` reflects latest values per system ③ Invalid JSON payloads are caught and logged, not crash the server ④ MQTT client reconnects automatically if broker connection drops ⑤ `soil` field contains live analog readings from ESP32 soil moisture sensor |
| **Edge Cases** | HiveMQ Cloud unreachable → MQTT client fails to connect at startup (FastAPI still starts, sensors show null). ESP32 offline → `sensor_state` retains last known values. Malformed JSON → `JSONDecodeError` caught, raises error string (should be logged, not re-raised — known minor issue). Broker connection drops → paho `loop_start()` handles reconnection automatically. TLS certificate issues → currently using `setInsecure()` on ESP32 (TODO: cert pinning for production) |
| **Dependencies** | HiveMQ Cloud account with credentials in `.env`, ESP32 flashed with MQTT/TLS firmware, DHT11 (pin 18) + soil moisture sensor (pin 36) wired to ESP32 |
